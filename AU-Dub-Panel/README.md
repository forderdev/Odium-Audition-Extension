...
- **Üst üste binme bug'ı çözüldü.** v1.4.2 teşhisi ortaya çıkardı: `clip.startTime` **saniye değil, SAMPLE cinsindendir**. Saniye değeri (örn. 15.482) atanınca host bunu sample sayısı gibi yorumlayıp ~0'a yuvarlıyordu, bu yüzden tüm clip'ler 0 saniyeye yığılıyordu.
...
- v1.4.1/1.4.1 yerleştirmede clip'ler üst üste çıktı: `clip.startTime = X` ataması hata vermeden değeri kabul ediyor (read-back doğru) ama clip taşınmıyor olabilir - `startTime` salt-okunur bir host property'siyse ExtendScript atamayı sessizce gölgeliyor. Audition'da clip muhtemelen **playhead konumuna** ekleniyor.
...
- `app.openDocument` hem String hem File argümanını "Illegal Parameter type" ile reddetti (File.exists=true olmasına rağmen). Doğru argüman tipi bilinmiyordu; v1.4.4/1.4.5 probe'u app metodlarını yalnızca isim olarak dökmüştü, imza olarak değil.
...
- Probe sonucu: `app.openDocument(String)` "Illegal Parameter type" atıyor - **File nesnesi** istiyor. v1.4.7'de string verilince WAV açılmıyor, fallback yanlışlıkla multitrack belgesini alıyor, dolayısıyla `audioClips.add(multitrackDoc)` clip döndürmüyordu.
